...
## 11. Unified Results Summary (Current State)

### Metamorphic Tests: 7/7 ✅

| ID | Relation | Endpoint | Статус |
|----|----------|----------|--------|
| MET-001 | Login case insensitivity | `/auth/login` | ✅ |
| MET-002 | Query param order independence | `/posts` | ✅ |
| MET-003 | Follow/unfollow symmetry | `/users/{username}/follow` | ✅ |
| MET-004 | Pagination consistency | `/posts?page=1&per_page=10` | ✅ |
| MET-005 | Search synonym equivalence | `/search` | ✅ |
| MET-006 | Profile update idempotency | `/users/{username}` | ✅ |
| MET-007 | Post creation-deletion cycle | `/posts` | ✅ |

### Mutation Tests: 47/49 ✅

| Category | Tests | Pass | Fail | Bugs Found |
|----------|-------|------|------|------------|
| API Response (base) | 8 | 8 | 0 | 0 |
| API Response (extended) | 15 | 15 | 0 | 0 |
| DB Data Mutation | 8 | 6 | 2 | BUG-005 (XSS), BUG-006 (negative likes) |
...
| Assertion Type | Count | Kills Mutants | Recommendation |
|----------------|-------|---------------|----------------|
| `toBeVisible()` | 13 | ⚠️ Sometimes | Use only for error states |
| `toHaveText()` | 3 | ✅ Always | Prefer for text fields |
| `toBeTruthy()` | 5 | ✅ Always | Good for existence checks |
| `toBeGreaterThan()` | 2 | ✅ Always | Good for numeric validation |
| `toContainText()` | 0 | — | Underutilized — add more |
...
1. **XSS vulnerability** (BUG-005): Post content renders HTML without escaping — Critical
2. **Negative likes** (BUG-006): `likes_count = -5` displays as-is — Low
3. **`toBeVisible()` overused**: 13 of 56 assertions — potential High Score Fallacy
4. **HOM tests effective**: Combined mutations (feed empty + likes zeroed) catch issues FOM miss

### Recommendations

1. **Replace 50% of `toBeVisible()`** with `toHaveText()` or `toHaveCount()` in mutation tests
2. **Fix BUG-005** (XSS) — highest priority security issue
3. **Add `toHaveCount()` assertions** for list-based mutations (items → [])
4. **Run mutation suite nightly** in CI, not per-PR (49 tests × 4 browsers = 196 runs)
